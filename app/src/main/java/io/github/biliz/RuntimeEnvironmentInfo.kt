...
object RuntimeEnvironmentInfo {
    ...
    const val EXTRA_RUNTIME_VALUES = "io.github.biliz.extra.RUNTIME_VALUES"
    ...
    private fun runtimeSnapshotValues(
        ...
        return linkedMapOf(
            ...
            ModuleSettings.KEY_RUNTIME_NIGHT_MODE to isHostNightMode(hostContext).toString(),
        ).apply {
            putAll(symbolScanStatusValues(hostContext))
        }
    }

    fun isHostNightMode(context: Context): Boolean {
        ...
        val candidatePrefs = listOf("bili_preference", "theme", "theme_preference", "bili_theme")
        for (prefName in candidatePrefs) {
            val sp = runCatching { context.getSharedPreferences(prefName, Context.MODE_PRIVATE) }.getOrNull() ?: continue
            val themeKey = runCatching { sp.getInt("theme_entries_last_key", 0) }.getOrDefault(0)
            if (themeKey == 2) return true
            val themeStr = runCatching { sp.getString("theme_entries_last_key", "") }.getOrNull()
            if (themeStr?.contains("night", ignoreCase = true) == true || themeStr?.contains("dark", ignoreCase = true) == true) return true
            val isNightMode = runCatching { sp.getBoolean("night_mode", false) }.getOrDefault(false)
