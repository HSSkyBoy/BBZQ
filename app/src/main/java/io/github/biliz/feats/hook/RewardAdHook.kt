...
import io.github.biliz.feats.symbol.RestoredRewardAdSymbols
...
class RewardAdHook(env: RoamingEnv) : BaseRoamingHook(env) {
    ...
    private fun scheduleSkipSweep(target: Any?) {
        ...
        if (!isRewardActivity(activity)) return
    ...
    private fun shouldClickText(activity: Activity, rawText: CharSequence?): Boolean {
        ...
        if (compact.contains("跳过") ||
            ...
            compact.contains("放弃奖励") ||
            compact.contains("知道了") ||
            compact.contains("关闭")
        ) {
            return true
        }
        return isRewardActivity(activity) &&
            (compact.contains("继续观看") || compact.contains("坚持退出"))
    }
    ...
    private fun isRewardActivity(activity: Activity): Boolean {
        ...
        return name.startsWith("com.bilibili.ad.reward.") ||
            name.contains("Reward", ignoreCase = true) ||
            name.contains("AdActivity", ignoreCase = true) ||
            name.contains("UnionActivity", ignoreCase = true)
    }
