...
internal class HighestBitrateProcessor(
    ...
    fun readStats(response: Any?): VideoStreamStats? {
        ...
        return runCatching {
            ...
            val stats = readSelectedStats(vodInfo) ?: readSelectedStats(videoInfo) ?: readSelectedStats(viewInfo)
            if (stats != null) {
                QualityState.preferredQuality = stats.quality
                PlayerTrace.logPlayViewResponse(
                    bvid = QualityState.currentBvid,
                    selectedQn = stats.quality,
                    availableQns = listOf(stats.quality),
                    reordered = false,
                    vipCleared = false,
                )
            }
    ...
    private fun reorderStreams(container: Any?) {
        ...
        val beforeQns = original.mapNotNull { it.callMethod("getStreamInfo")?.let { info -> number(info, "getQuality") } }
        QualityState.availableQualities = beforeQns
        ...
        val preferredQuality = original.asSequence()
        ...
        if (preferredQuality != null) {
            invokeNumber(container, "setQuality", preferredQuality)
            QualityState.preferredQuality = preferredQuality
        }
        ...
        if (reordered.indices.all { reordered[it] === original[it] }) {
            PlayerTrace.logPlayViewResponse(
                bvid = QualityState.currentBvid,
                selectedQn = preferredQuality,
                availableQns = beforeQns,
                reordered = false,
                vipCleared = true,
            )
            return
        ...
        val afterQns = reordered.mapNotNull { it.callMethod("getStreamInfo")?.let { info -> number(info, "getQuality") } }
        logInfo("reorderStreams: preferredQuality=$preferredQuality, before=$beforeQns, after=$afterQns")
        PlayerTrace.logPlayViewResponse(
            bvid = QualityState.currentBvid,
            selectedQn = preferredQuality,
            availableQns = beforeQns,
