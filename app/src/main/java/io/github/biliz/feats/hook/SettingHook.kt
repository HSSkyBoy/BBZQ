...
import io.github.biliz.ModuleSettingsNavigator
...
import io.github.biliz.feats.methodsNamed
...
class SettingHook(env: RoamingEnv) : BaseRoamingHook(env) {
    ...
    private fun replaceJoinUsEntry(fragment: Any) {
        ...
        val listener = Proxy.newProxyInstance(listenerType.classLoader, arrayOf(listenerType)) { proxy, method, args ->
            when (method.name) {
                "onPreferenceClick" -> {
                    ModuleSettingsNavigator.open(activity, runtimeSnapshot(activity))
                    true
                }
                "equals" -> proxy === args?.getOrNull(0)
                "hashCode" -> System.identityHashCode(proxy)
                "toString" -> "BilizOnPreferenceClickListener"
    ...
    private fun runtimeSnapshot(activity: Activity? = null) =
        RuntimeEnvironmentInfo.runtimeSnapshotBundle(
            hostContext = activity ?: env.hostContext,
            ...
            val isNight = RuntimeEnvironmentInfo.isHostNightMode(activity ?: env.hostContext)
            putString(ModuleSettings.KEY_RUNTIME_NIGHT_MODE, isNight.toString())
            prefs.edit().putString(ModuleSettings.KEY_RUNTIME_NIGHT_MODE, isNight.toString()).apply()
        }

    private companion object {
        private val JOIN_US_KEYS = arrayOf("JoinUs", "pref_key_joinus")
        private const val ENTRY_TITLE = "高级设置"
        private const val ENTRY_SUMMARY = "BILIZ 设置"
