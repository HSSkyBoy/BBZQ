...
/**
 * 视频播放结束界面定制 Hook：
 * 绝不侵入或破坏任何底层播放调度与 Kotlin 协程状态机，播放与切集逻辑 100% 保持官方原生。
 *
 * 核心功能：
 * 1. 方案 A：移花接木纯进化结束推荐，若 Card 0 是推广广告，自动以真正的普通视频替换，绝不展示广告，绝不留白空槽；
 * 2. 去除半屏结束推荐：通过将半屏 Widget View 置为 GONE 优雅隐藏，保留居中原生重播键，放行全屏推荐；
 * 3. 去除全部结束推荐：半屏设为 GONE，同时阻断官方连播倒计时弹窗；
 * 4. 启用实验结束推荐：通过官方枚举合法设置命中 GROUP_1 实验组。
 */
class VideoEndRecommendHook(env: RoamingEnv) : BaseRoamingHook(env) {
    ...
    override fun startHook() {
        ...
        var installed = 0
        installed += installEndPageExpServiceHook()
    ...
    // =========================================================================
    // 2. Compose 实验连播倒计时分组控制（仅在明确开启实验组时合法赋值，零破坏）
    // =========================================================================

    private fun installEndPageExpServiceHook(): Int {
        var count = 0
        val expServiceClass = classLoader.findClassOrNull("com.bilibili.ship.theseus.ugc.endpage.UGCEndPageExperimentService") ?: return 0
        val groupEnumClass = classLoader.findClassOrNull("com.bilibili.playerbizcommonv2.utils.PlayerContinuousPlayGroup") ?: return 0

        val group1Value = runCatching { java.lang.Enum.valueOf(groupEnumClass.asSubclass(Enum::class.java), "GROUP_1") }.getOrNull()

        expServiceClass.declaredConstructors.forEach { constructor ->
            env.hookAfter(constructor) { param ->
                runCatching {
                    val instance = param.thisObject ?: return@runCatching
                    if (ModuleSettings.isEnableExpEndPage(prefs)) {
                        if (group1Value != null) setExperimentGroup(instance, groupEnumClass, group1Value)
                    }
                }
            }
            count++
    ...
    private fun setExperimentGroup(instance: Any, groupEnumClass: Class<*>, targetValue: Any) {
        runCatching {
            val cls = instance.javaClass
            val fieldN = cls.allFields().firstOrNull { it.type == groupEnumClass || it.name == "N" }
            fieldN?.isAccessible = true
            fieldN?.set(instance, targetValue)
        }
    }
