...
import io.github.biliz.feats.symbol.RestoredVideoCommentSymbols
...
class VideoCommentHook(env: RoamingEnv) : BaseRoamingHook(env) {
    ...
    private fun hookDisableComment(symbols: RestoredVideoCommentSymbols): Int {
        if (!ModuleSettings.isCommentDisableEnabled(prefs)) return 0
        ...
        symbols.headerDecorativeMethods.forEach { method ->
            env.hookBefore(method) { param ->
                runCatching {
                    ...
                    val items = param.args.firstOrNull() as? List<Any?> ?: return@runCatching
                    items.forEach { item ->
