...
internal class VideoStatsOverlayController(
    ...
    fun update(stats: VideoStreamStats) {
        val prev = latestStats
        if (prev != null && prev.quality != stats.quality) {
            PlayerTrace.logQualitySwitch(prev.quality, stats.quality, "Active stream changed")
        }
        latestStats = stats
        mainHandler.post {
                ...
                addView(
                    ScrollView(activity).apply {
                        ...
                        val root = LinearLayout(activity).apply {
                            ...
                            val traces = PlayerTrace.getRecentTraces()
                            if (traces.isNotEmpty()) {
                                addView(sectionTitle(activity, "画质链路追踪日志", 16f))
                                traces.takeLast(16).forEach { entry ->
                                    val tv = TextView(activity).apply {
                                        text = entry.toString()
                                        textSize = 10f
                                        setTextColor(if (entry.isWarning) Color.parseColor("#FFA726") else Color.parseColor("#BBBBBB"))
        ...
        runCatching {
            ...
            val downloadContainer = LinearLayout(activity).apply {
                ...
                if (bvid.isNullOrEmpty()) {
                    ...
                    io.github.biliz.feats.download.VideoDownloadManager.fetchVideoInfo(activity, bvid, cookies) { list, error ->
                            ...
                            list.forEach { quality ->
                                val btn = android.widget.Button(activity).apply {
                                    ...
                                    setOnClickListener {
                                        ...
                                        io.github.biliz.feats.download.VideoDownloadManager.downloadAndMux(activity, bvid, quality.videoUrl, quality.audioUrl) { msg, pct ->
