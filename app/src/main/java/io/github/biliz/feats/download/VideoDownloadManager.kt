package io.github.biliz.feats.download
...
object VideoDownloadManager {
    ...
    fun fetchVideoInfo(
        ...
        val capturedCid = io.github.biliz.feats.hook.VideoStatsOverlayController.currentCid
        val capturedBvid = io.github.biliz.feats.hook.VideoStatsOverlayController.currentBvid
            ...
            override fun onResponse(call: Call, response: Response) {
                response.use {
                    ...
                    try {
                        ...
                        android.util.Log.e("BILIZ", "fetchPlayUrl failed", e)
                        e.printStackTrace()
    ...
    fun downloadAndMux(
        ...
        Thread {
            ...
            val tempDir = File(activity.cacheDir, "biliz_dl").apply { mkdirs() }
            val videoFile = File(tempDir, "${bvid}_video_$taskId.m4s")
            val audioFile = File(tempDir, "${bvid}_audio_$taskId.m4s")
            val publicDir = File(android.os.Environment.getExternalStoragePublicDirectory(android.os.Environment.DIRECTORY_DOWNLOADS), "BILIZ").apply { mkdirs() }
            ...
            try {
                ...
                android.util.Log.e("BILIZ", "downloadAndMux failed", e)
                e.printStackTrace()
