package io.github.biliz.feats.symbol
...
import io.github.biliz.feats.symbol.dexkit.DexKitBridgeProvider
import io.github.biliz.feats.symbol.dexkit.scanMessage
...
object BiliSymbolResolver {
    const val CACHE_PREFS_NAME = "biliz_symbol_cache"
    ...
    fun resolve(
        ...
        if (isMainThread) {
            ...
            kotlin.concurrent.thread(name = "BILIZ-AsyncScan", isDaemon = true) {
                runCatching {
    ...
    private fun scanRewardAd(classLoader: ClassLoader): SymbolScanResult<RewardAdSymbols> {
        ...
        val activityOnCreate = activityClass?.findMethod("onCreate", Void.TYPE, Bundle::class.java)
            ?.takeIf { it.declaringClass == activityClass }
    ...
    private fun scanVideoDetailBannerAd(
        classLoader: ClassLoader,
        @Suppress("UNUSED_PARAMETER") bridge: () -> DexKitBridge?,
        ...
        val symbols = VideoDetailBannerAdSymbols(
            ...
            relateGameComponentTypeName = null,
            simpleViewEntryConstructor = null,
            createViewEntry = null,
            bindToView = null,
            kotlinUnitField = null,
            evidence = "proxy=$hasProxyHook,pausedRequest=$hasPausedPageRequestHook,pausedPanel=$hasPausedPagePanelHook",
        )
        val hookPoints = listOf(
            childHookPoint(
            ...
            childHookPoint(
                ...
                true,
                "relate game hook handled via protobuf relate filter hook",
                "filtered via protobuf relate filter hook",
            ),
        )
    ...
    private fun scanVideoDetailRelateFeed(
        ...
        if (responseGetItems.isEmpty()) {
            return SymbolScanResult.Missing("VideoDetailRelate response getItems methods not found")
        }

        val symbols = VideoDetailRelateFeedSymbols(
            responseGetItems = responseGetItems,
            detailRelateServiceMethod = null,
            evidence = "responses=${responseGetItems.size}",
        )
        val target = responseGetItems.joinToString("|") { it.getItems.declaringClassName }
    ...
    private fun scanSkipVideoAdProgress(
        ...
        val playerSeekAttach = PLAYER_SEEK_WIDGET_CLASSES
            .asSequence()
            .mapNotNull { classLoader.loadClassOrNull(it) }
            .flatMap { type ->
                type.allMethods().asSequence().filter {
        ...
        if (progressOnDraw == null && playerSeekAttach.isEmpty() && storyOnStart.isEmpty() && inlineUpdate.isEmpty()) {
        ...
        val symbols = SkipVideoAdProgressSymbols(
            ...
            playerSeekAttachMethods = playerSeekAttach.map(MethodDescriptor::of),
            ...
            evidence = "draw=${progressOnDraw != null},seekAttach=${playerSeekAttach.size},story=${storyOnStart.size},inline=${inlineUpdate.size},panel=${panelWidget != null}",
        )
        val hookPoints = listOf(
            childHookPoint(HP_SKIP_VIDEO_AD_PROGRESS_DRAW, playerSeekAttach.isNotEmpty() || progressOnDraw != null, "player seek attach or onDraw hook not found"),
    ...
    private val PLAYER_SEEK_WIDGET_CLASSES = setOf(
        "com.bilibili.playerbizcommonv2.widget.seek.v3.PlayerSeekWidget3",
        "com.bilibili.lib.playerbizcommon.widget.PlayerSeekWidget3",
    )
