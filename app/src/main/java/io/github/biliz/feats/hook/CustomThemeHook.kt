...
import io.github.biliz.feats.fieldOrNull
import io.github.biliz.feats.hookBefore
import io.github.biliz.feats.newInstanceOrNull
import io.github.biliz.feats.replace
import io.github.biliz.feats.setBooleanField
import io.github.biliz.feats.setIntField
import io.github.biliz.feats.setObjectField
import io.github.biliz.feats.symbol.RestoredCustomThemeSymbols
 ...
 * BiliRoaming while keeping all settings in BILIZ's remote preferences.
 */
    ...
    private fun suppressThemeReset(symbols: RestoredCustomThemeSymbols) {
        symbols.themeResetMethods.forEach { method ->
            env.replace(method) { param ->
                ...
                var isPostCreate = false
                for (i in 0 until limit) {
                    val frame = trace[i]
                    if (frame.className == MAIN_ACTIVITY && frame.methodName == "onPostCreate") {
                        isPostCreate = true
                        break
                    }
                }
